#!/bin/bash
# Parallel whois check for V2 candidates (500 domains)
set -u
cd "$(dirname "$0")"

INPUT="top500-v2.txt"
OUT_AVAIL="available-v2.txt"
OUT_TAKEN="taken-v2.txt"
OUT_ERROR="errors-v2.txt"
OUT_RAW="raw-results-v2.txt"
...
check_domain() {
  ...
  if echo "$response" | grep -qiE "no match|not found|no data found|available|is free|^[[:space:]]*$"; then
    echo "AVAILABLE|$domain" >> "$OUT_RAW"
    echo "$domain" >> "$OUT_AVAIL"
  elif echo "$response" | grep -qiE "^[[:space:]]*domain name:|registrar:|creation date:|registrant|Name Server:|status: clientTransferProhibited"; then
    echo "TAKEN|$domain" >> "$OUT_RAW"
    echo "$domain" >> "$OUT_TAKEN"
  else
    echo "UNKNOWN|$domain|$(echo "$response" | head -1 | tr -d '\r')" >> "$OUT_RAW"
