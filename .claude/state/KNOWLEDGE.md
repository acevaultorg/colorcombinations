...
- **/palette-of-the-day** — retention hook (v17.2 craft ship). Today's palette chosen deterministically from all 378 plates via days-since-2026-01-01 mod 378. Homepage hero card + 14-day archive grid. Deep-linkable. JSON-LD CreativeWork + BreadcrumbList. Cycle length is slightly over a year, so same-date-next-year surfaces the same plate (intentional — stable permalinks). `src/data/palette-of-the-day.ts` is the rotation primitive; `src/components/PaletteOfDay.astro` is the homepage card; `src/pages/palette-of-the-day/index.astro` is the archive. <!-- verified: 2026-04-17 -->
- **/guide/wada-palette-primer** — pillar distribution content (v17.3 god ship). 1000-word editorial primer targeting "sanzo wada color palette guide" long-tail + LLM-citation extractability. Names three design principles as framework ("dominant/voice/breath" · "temperature contrast" · "desaturated gearing") — quotable, moated. 3 example palettes embedded as PaletteCard components linked to detail pages. 5-step decision-tree picking guide with internal links to /collections/, /tools/, /browse/. JSON-LD Article + BreadcrumbList. Love Score 0.76 · Distribution Fit 0.74. `src/pages/guide/wada-palette-primer.astro`. <!-- verified: 2026-04-17 -->
